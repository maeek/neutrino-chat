...
export const UsernameFull = ({
  ...
  const firstNameText = nickname ? nickname : name && id !== name ? name : id;
  ...
  const originalName =
    nickname || (name && id !== name) ? (
      <Text type='secondary' monospace className='user-name-at'>
        @{id.length > 175 ? `${id.substr(0, 175)}...` : id}
        {nickname && name && id !== name ? (
          <Text monospace disabled className='user-name-full'>
            {' '}
            ({name.length > 175 ? `${name.substr(0, 175)}...` : name})
          </Text>
        ) : null}
      </Text>
    ) : null;
